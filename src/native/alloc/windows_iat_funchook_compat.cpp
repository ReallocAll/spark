...
bool loadShim(ShimApi &api, std::string &error)
{
    ...
    if (api.module == nullptr) {
        ...
        while (!module_directory.empty() && (module_directory.back() == L'\\' || module_directory.back() == L'/')) {
            module_directory.pop_back();
        }
        const std::size_t separator = module_directory.find_last_of(L"/\\");
        const std::wstring leaf =
            separator == std::wstring::npos ? module_directory : module_directory.substr(separator + 1);
        ...
        const std::wstring fallback_path = module_directory.substr(0, separator + 1) + KShimFileName;
        ...
        if (api.module == nullptr) {
            error = "LoadLibraryW(spark_allocation_shim.dll) failed from Endstone shadow-copy directory: " +
                    std::to_string(primary_error) +
                    "; plugins parent fallback failed: " + std::to_string(::GetLastError());
            return false;
        }
    }
...
{
    return "native-ucrt/funchook";
}

extern "C" const char *funchook_backend_name(void)
{
    return "Windows UCRT/funchook";
}
